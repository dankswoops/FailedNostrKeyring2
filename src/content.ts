...
window.addEventListener('message', async (event: MessageEvent) => {
  ...
  if (event.data.type === 'NOSTR_GET_PUBLIC_KEY') {
    ...
    try {
      ...
      window.postMessage({ type: 'NOSTR_PUBLIC_KEY_RESPONSE', error: (error as Error).message }, '*');
    }
    ...
    try {
      ...
      window.postMessage({ type: 'NOSTR_SIGNED_EVENT_RESPONSE', error: (error as Error).message }, '*');
    }
    ...
    try {
      ...
      window.postMessage({ type: 'NOSTR_ENCRYPTED_MESSAGE_RESPONSE_NIP04', error: (error as Error).message }, '*');
    }
    ...
    try {
      ...
      window.postMessage({ type: 'NOSTR_DECRYPTED_MESSAGE_RESPONSE_NIP04', error: (error as Error).message }, '*');
    }
    ...
    try {
      ...
      window.postMessage({ type: 'NOSTR_ENCRYPTED_MESSAGE_RESPONSE_NIP44', error: (error as Error).message }, '*');
    }
    ...
    try {
      ...
      window.postMessage({ type: 'NOSTR_DECRYPTED_MESSAGE_RESPONSE_NIP44', error: (error as Error).message }, '*');
    }
  }
});
